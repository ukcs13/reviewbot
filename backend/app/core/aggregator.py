...
def aggregate(results: list[AgentResult]) -> AggregatedReview:
        ...
        return AggregatedReview(
            issues=[
                IssueBase(
                    ...
                    message=(
                        "The review pipeline was unable to process your request because "
                        "the AI service quota has been reached. Please try again in a "
                        "few minutes."
                    ),
        ...
        return AggregatedReview(
            issues=[
                IssueBase(
                    ...
                    message=(
                        "The review pipeline could not fetch source files from this "
                        "repository. The repository may be private, empty, or the "
                        "GitHub API rate limit was hit. Try again with a different "
                        "public repository or upload a ZIP file."
                    ),
    ...
    if score >= 85:
        ...
        summary = (
            "This project demonstrates excellent code quality and architecture "
            "with minimal issues."
        )
    elif score >= 70:
        decision = ReviewDecision.good
        summary = (
            "Overall solid implementation with some room for improvement in "
            "specific areas."
        )
    elif score >= 50:
        decision = ReviewDecision.needs_work
        summary = (
            "Significant issues found. Recommendations should be addressed "
            "before production."
        )
    else:
        decision = ReviewDecision.critical
        summary = (
            "Critical vulnerabilities or architectural flaws detected. "
